...
class _EditMovieScreenState extends ConsumerState<EditMovieScreen> {
  ...
  void _initializeFields() {
    ...
    _oscarsController.text = movie.oscarsCount?.toString() ?? '';
    _lengthController.text = movie.length.toString();
    _totalBoxOfficeController.text = movie.totalBoxOffice?.toString() ?? '';
    
    _coordinatesXController.text = movie.coordinates.x.toString();
    _coordinatesYController.text = movie.coordinates.y.toString();
    
    _selectedGenre = movie.genre;
    
    // Режиссер
    _directorNameController.text = movie.director.name;
    _directorPassportController.text = movie.director.passportID;
    ...
    _directorLocationXController.text = movie.director.location.x.toString();
    _directorLocationYController.text = movie.director.location.y.toString();
    _directorLocationZController.text = movie.director.location.z.toString();
    
    // Оператор
    if (movie.operator != null) {
      _hasOperator = true;
      _operatorNameController.text = movie.operator!.name;
      _operatorPassportController.text = movie.operator!.passportID;
      ...
      _operatorLocationXController.text = movie.operator!.location.x.toString();
      _operatorLocationYController.text = movie.operator!.location.y.toString();
      _operatorLocationZController.text = movie.operator!.location.z.toString();
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Редактировать фильм'),
      ),
      body: _isLoading
  ...
  Future<void> _updateMovie() async {
    ...
    try {
      ...
      await ref.read(movieListNotifierProvider.notifier).updateMovie(widget.movie.id, movieRequest);
